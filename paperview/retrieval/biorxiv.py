...
def get_all_content_details_by_interval(interval: str, server: str='bioRxiv', format: str='json') -> List[ArticleDetail]:
    """Retrieve all pages of results for the given interval."""
    all_results = []
    cursor = 0
    while True:
        results = query_content_detail_by_interval(interval, cursor=cursor, server=server, format=format)
