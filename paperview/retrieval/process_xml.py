# took from here
# https://github.com/sibils/jats-parser/blob/master/process_xml.py
...
def get_fig_parents(fname, someroot):
    ...
    if figs is not None:
        for fig in figs:
            ...
            if parents.get(parent_tag) is None:
                parents[parent_tag] = 0
...
def get_pub_date_by_type(someroot, selector, pubtype, format):
    ...
    if len(dates) == 0:
        return {'date': None, 'status': 'not found'}
    dt = dates[0]
    ...
    year = year.strip()
    if len(year) == 0:
        return {'date': None, 'status': 'incomplete'}
    mnode = dt.find('month')
    mm = '01'
...
def build_medlinePgn(fp, lp):
    if fp != None and len(fp) > 0 and lp != None and len(lp) > 0:
        return fp + '-' + lp
    if fp != None and len(fp) > 0:
        return fp + '-?'
    if lp != None and len(lp) > 0:
        return '?-' + lp
    return ''
...
def get_authors(someroot):
    ...
    for a in authors:
        ...
        for el in a.iter():
            if el.tag == 'surname':
                if el.text != None:
                    surname = clean_string(el.text)
            elif el.tag == 'given-names':
                if el.text != None:
                    givennames = clean_string(el.text)
            # affiliations
            elif el.tag == 'xref' and el.get('ref-type') == 'aff':
                if el.get('rid') != None:
                    affiliation_list.append(el.get('rid'))
            # affiliations (alternative)
            elif el.tag == 'aff':
                if el.text != None:
                    affiliation_list.append(clean_string(el.text))
    ...
    if len(result) == 0:
        file_status_add_error("WARNING: no authors")
    return result
...
def remove_alternative_title_if_redundant(someroot):
    ...
    if ttl is not None and alt is not None:
...
def handle_table_wrap(pmcid, tw):
    ...
    if table_tree is None:
        table_tree = tw.find('alternatives/table')
...
def handle_table_wrap_group_elements(someroot):
    ...
    for g in g_list:
        ...
        g_captions = []
        for gc in g.iterchildren('caption'):
            g_captions.append(' '.join(gc.itertext()))
        g_labels = []
        for gl in g.iterchildren('label'):
            g_labels.append(' '.join(gl.itertext()))
...
def handle_list(list):
    ...
    if tail is not None:
        contentList.append({'tag': 'p', 'text': tail})
    return contentList
...
def handle_paragraph(pmcid, el):
    ...
    if ptext is not None and ptext != '':
        contentList.append({'tag': 'p', 'text': ptext})
    ...
    if ptail is not None and ptail != '':
        contentList.append({'tag': 'p', 'text': ptail})
    return contentList
...
def handle_section_flat(pmcid, sec, level, implicit, block_id):
    ...
    for el in sec:
        ...
        if isinstance(el, etree._Comment):
            continue
        if isinstance(el, etree._XSLTProcessingInstruction):
            continue
        ...
        addContentsOrWrappedContents(
            sectionList, mainSection, contentsToBeAdded, level, terminalContentShouldBeWrapped
        )
...
# sub_sec : [p2]
# wrap_sec: [p3]
def addContentsOrWrappedContents(
    sectionList, currentSection, contentsToBeAdded, level, shouldBeWrapped
):
    if contentsToBeAdded == []:
        return
...
def parse_PMC_XML_core(xmlstr, root, input_file):
    ...
    for xs in root.xpath('//xref/sup'):
        xs.getparent().remove(xs)
    for sx in root.xpath('//sup/xref'):
        sx.getparent().remove(sx)
    ...
    for el in root.xpath('/article/floats-wrap'):
        el.tag = 'floats-group'
    ...
    dict_doc['medline_ta'] = get_text_from_xpath(
        root, '/article/front/journal-meta/journal-id', False, True
    )

    dict_doc['journal'] = get_multiple_texts_from_xpath(
        root, '/article/front/journal-meta//journal-title', True
    )
    ...
    dict_doc['title'] = get_multiple_texts_from_xpath(
        root, '/article/front/article-meta/title-group', True
    )
    dict_doc['pmid'] = get_text_from_xpath(
        root, '/article/front/article-meta/article-id[@pub-id-type="pmid"]', True, False
    )
    dict_doc['doi'] = get_text_from_xpath(
        root, '/article/front/article-meta/article-id[@pub-id-type="doi"]', True, False
    )
    # the archive and manuscript types are used for preprints
    dict_doc['archive_id'] = get_text_from_xpath(
        root, '/article/front/article-meta/article-id[@pub-id-type="archive"]', True, False
    )
    dict_doc['manuscript_id'] = get_text_from_xpath(
        root, '/article/front/article-meta/article-id[@pub-id-type="manuscript"]', True, False
    )

    # we might find at least one of these:
    pmc1 = get_text_from_xpath(
        root, '/article/front/article-meta/article-id[@pub-id-type="pmc-uid"]', True, False
    )
    pmc2 = get_text_from_xpath(
        root, '/article/front/article-meta/article-id[@pub-id-type="pmc"]', True, False
    )
    pmc = pmc1
    if pmc == '':
        pmc = pmc2
    if pmc == '' and dict_doc['archive_id'] == '':
        file_status_add_error("ERROR, no value for article id in types pmc-uid, pmc, or archive")
    dict_doc['pmcid'] = pmc
    dict_doc['_id'] = pmc
    ...
    dict_doc['volume'] = get_text_from_xpath(
        root, '/article/front/article-meta/volume', True, False
    )
...
def simplify_node(el, kept_tags, starting=True):
    if starting:
        ...
        if el.text is not None:
            setattr(trg_node, trg_attr, (getattr(trg_node, trg_attr) or '') + el.text)
        for subel in el.iterchildren():
            el.addnext(subel)
            simplify_node(subel, kept_tags, False)
        if el.tail is not None:
            setattr(trg_node, trg_attr, (getattr(trg_node, trg_attr) or '') + el.tail)
        el.getparent().remove(el)
...
def xmlstr_to_dict(xmlstr):
    xmlstr = cleanup_input_xml(xmlstr)
    root = etree.fromstring(xmlstr)
    dict_doc = parse_PMC_XML_core(xmlstr, root, input_file=None)
    return dict_doc


def main(input: str, output_file: str):

    file_status_reset()
    file_status_set_name(input)
    xmlstr = get_file_content(input)
    xmlstr = cleanup_input_xml(xmlstr)
    root = etree.fromstring(xmlstr)

    dict_doc = parse_PMC_XML_core(xmlstr, root, input)
...
def test2():
    parser = OptionParser()
    root = etree.XML(
        '<root><some>stuff before</some><fig-group><caption><p>fg caption</p></caption><fig><caption><p>fig 1 caption</p></caption></fig><fig id="totofig"><caption><p>fig 2 caption</p>something else</caption></fig></fig-group>1-hi there<child><a href="toto">2-toto href</a></child>3-something normal<b>4-something in bold</b>some tail</root>'
    )
